...
function chooseNumber() {
  let index = prompt("Wat is je favoriete film?!");
  setColor(parseInt(index));
}

function setColor(number) {
  document.querySelector(getClazzName(number)).classList.add("set");
}

function getClazzName(number) {
  console.log(topFilms[number - 1]);
  return ".film" + number + "-naam";
}
